...
japanese_set = getJapDataSet(dataset)
pad_on_right = True
max_length = 128
doc_stride = 64

japanese_set = japanese_set.remove_columns("language")

japanese_set = japanese_set.remove_columns("document_url")
...
tokenized_datasets = japanese_set.map(prepare_train_features, batched=False,
...
for epoch in range(args.epochs):
    ...
    for batch in train_loader:
        ...
        if (loss.item() < min_loss):
            min_loss = loss.item()
            max_acc = acc
            torch.save(model.state_dict(), 'model_week3_jap.pth')
        loss.backward()
        optimizer.step()
        batch_num += 1
...
model = QA_model(args.input_dim, args.hidden_dim, args.output_dim).to('cuda')
model.load_state_dict(torch.load("model_week3_jap.pth"))
