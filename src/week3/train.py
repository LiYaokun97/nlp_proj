...
# count=0
# count_acc=0
# for label ,question_input_ids,question_attention_mask,document_input_ids,document_attention_mask in valid_dl:
        
#         label = torch.squeeze(label,dim=-1)
#         predict_label = model (question_input_ids,question_attention_mask,document_input_ids,document_attention_mask)
#         pred = predict_label.max(-1,keepdim=True)[1]
#         acc = pred.eq(label.view_as(pred)).sum().item()/predict_label.shape[0]
#         count_acc +=acc
#         count+=1
#         max_acc = max(max_acc,acc)
# print("test max acc:",max_acc)
