...
class QA_model(nn.Module):
    def __init__(self,input_dim,hidden_dim,output_dim):
        ...
        self.output_layer = MLP(hidden_dim,output_dim)
    ...
    def forward(self,input):
        print("type : ", type(input["input_ids"]))
        input_ids = torch.stack(input["input_ids"],dim=0).cuda()
        print("input_ids shape:", input_ids.shape)
        attention_mask=torch.stack(input["attention_mask"],dim=0).cuda()
        ...
        hidden_state= self.transformer_layer(input_ids=input_ids, attention_mask=attention_mask)
        hidden_out = mean_pooling(hidden_state, attention_mask)
