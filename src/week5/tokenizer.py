...
def data_process_continuous(dataSet, SbertModel, tokenizer, tokenPart="document"):
    data = []
...
def data_process_transformer(dataSet, w2vModel, tokenizer, tokenPart="document"):
    if tokenPart == 'answer':
        data = []
    else:
    ...
    for element in dataSet:
        if tokenPart == "document":
            en_tensor_ = tokenize_function(element["document_plaintext"])

            data["input_ids"].append(torch.tensor(en_tensor_['input_ids']).cuda())
            data["attention_mask"].append(torch.tensor(en_tensor_['attention_mask']).cuda())
        elif tokenPart == "question":
            en_tensor_ = tokenize_function(element["question_text"])
    ...
    if tokenPart == 'answer':
        ...
        data["input_ids"] = torch.stack(data["input_ids"], dim=0)
        data["attention_mask"] = torch.stack(data["attention_mask"], dim=0)
    return data
...
def getEnglishData(data, tokenPart="document"):
    ...
    return data_process_transformer(englishDataSet, SbertModel, tokenizer, tokenPart)
