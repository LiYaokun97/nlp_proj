...
class QA_model(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        configuration.num_labels = self.num_labels
        self.transformer_layer = DistilBertForTokenClassification(configuration)

    def forward(self, input):
        input_ids = torch.stack(input["input_ids"], dim=1).cuda()
        attention_mask = torch.stack(input["attention_mask"], dim=1).cuda()
        labels = torch.stack(input['label'], dim=1).cuda()
        ...
        hidden_states = self.transformer_layer(input_ids=input_ids, attention_mask=attention_mask,
                                               output_hidden_states=True, return_dict=True)
        # hidden_out = mean_pooling(hidden_states[-1], attention_mask)
        predict_label = hidden_states['logits']
